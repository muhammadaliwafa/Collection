    ...
    for(int i=0; i <=7; i++){
      if (i==0){
          if((nowH.hM==9 && floatnow >= sholatT[0]) && (floatnow < (sholatT[0]+0.1/60))){
            RunSel=97;
            if(!digitalRead(3)){ // apakah dfplayer sedang sibuk
              mp3_set_volume (10); // maksimum 30
              mp3_play(4);
            }
//                delay(100);
           }
     ...
     {if (i==2)
      ...
      {if((floatnow >= (sholatT[i]-20.14/60)) && (floatnow < (sholatT[i]-20.04/60)))
       {if (i!=0 and i!=2 and i!=3)
      {RunSel=99;
      if(!digitalRead(3)){ // apakah dfplayer sedang sibuk
        mp3_set_volume (10); // maksimum 30
